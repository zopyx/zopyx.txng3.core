...
text_de = 'Bei den dreitägigen Angriffen seien auch bis auf einen alle the Flugplätze der Taliban zerstört worden'
...
class LexiconTests(unittest.TestCase):
    ...
    def testWids(self):
        ...
        for word, wid in d.items():
            self.assertEqual(
                self._lexicon.getWordAndLanguage(wid), (word, 'de'))
    ...
    def testRightTruncation(self):
        ...
        def _check(term, expected, language='en'):
            ...
            r2 = list(expected)
            r2.sort()
            self.assertEqual(r1, r2, 'got: %s, expected %s' %
                             (repr(r1), repr(r2)))
    ...
    def testSimilaritySearch(self):
        ...
        def _check(term, expected, language='en'):
            r1 = list([term for term, ratio in M(
                term, 0.5, language=language)])
            r1.sort()
            r2 = list(expected)
            r2.sort()
            self.assertEqual(r1, r2, 'got: %s, expected %s' %
