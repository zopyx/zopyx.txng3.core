    ...
    def writetab(self, tabfile):
        ...
        for key, lre in self.lexstatere.items():
            ...
            for i in range(len(lre)):
                titem.append((self.lexstateretext[key][
                             i], _funcs_to_names(lre[i][1])))
            tabre[key] = titem
    ...
    def token(self):
        ...
        while lexpos < lexlen:
            ...
            for lexre, lexindexfunc in self.lexre:
                ...
                # Set last match in lexer so that rules can access it if they
                # want
                self.lexmatch = m
    ...
    for l in lines:
        ...
        if m:
            ...
            if not prev:
                ...
                print >>sys.stderr, "%s:%d: Rule %s redefined. Previously defined on line %d" % (
...
def _statetoken(s, names):
    ...
    for i in range(1, len(parts)):
        if not names.has_key(parts[i]) and parts[i] != 'ANY':
            break
    if i > 1:
...
def lex(module=None, object=None, debug=0, optimize=0, lextab="lextab", reflags=0, nowarn=0):
    ...
    try:
        for c in literals:
            if not (isinstance(c, types.StringType) or isinstance(c, types.UnicodeType)) or len(c) > 1:
                print >>sys.stderr, "lex: Invalid literal %s. Must be a single character" % repr(
                    c)
                error = 1
                continue
        ...
        if not (isinstance(states, types.TupleType) or isinstance(states, types.ListType)):
            ...
            for s in states:
                if not isinstance(s, types.TupleType) or len(s) != 2:
                    print >>sys.stderr, "lex: invalid state specifier %s. Must be a tuple (statename,'exclusive|inclusive')" % repr(
                        s)
                    error = 1
                    continue
                name, statetype = s
                if not isinstance(name, types.StringType):
                    print >>sys.stderr, "lex: state name %s must be a string" % repr(
                        name)
                    error = 1
                    continue
    ...
    for f in funcsym.values():
        f.sort(lambda x, y: cmp(x[1].func_code.co_firstlineno, y[
               1].func_code.co_firstlineno))
        ...
        for fname, f in funcsym[state]:
            ...
            if not optimize:
                ...
                if ismethod:
                    ...
                    reqargs = 1
                if nargs > reqargs:
                    print >>sys.stderr, "%s:%d: Rule '%s' has too many arguments." % (
                        file, line, f.__name__)
                    error = 1
                    continue

                if nargs < reqargs:
                    print >>sys.stderr, "%s:%d: Rule '%s' requires an argument." % (
                        file, line, f.__name__)
                    error = 1
                    continue

                if tokname == 'ignore':
                    print >>sys.stderr, "%s:%d: Rule '%s' must be defined as a string." % (
                        file, line, f.__name__)
                    error = 1
                    continue
            ...
            if f.__doc__:
                if not optimize:
                    try:
                        c = re.compile("(?P<%s>%s)" % (
                            f.__name__, f.__doc__), re.VERBOSE | reflags)
                        if c.match(""):
                            print >>sys.stderr, "%s:%d: Regular expression for rule '%s' matches empty string." % (
                                file, line, f.__name__)
                            error = 1
                            continue
                    except re.error as e:
                        print >>sys.stderr, "%s:%d: Invalid regular expression for rule '%s'. %s" % (
                            file, line, f.__name__, e)
                        if '#' in f.__doc__:
                            print >>sys.stderr, "%s:%d. Make sure '#' in rule '%s' is escaped with '\\#'." % (
                                file, line, f.__name__)
                        error = 1
                        continue
                ...
                print >>sys.stderr, "%s:%d: No regular expression defined for rule '%s'" % (
                    file, line, f.__name__)
        ...
        for name, r in strsym[state]:
            ...
            if not optimize:
                ...
                if not lexobj.lextokens.has_key(tokname) and tokname.find("ignore_") < 0:
                    print >>sys.stderr, "lex: Rule '%s' defined for an unspecified token %s." % (
                        name, tokname)
                    error = 1
                    continue
                try:
                    c = re.compile("(?P<%s>%s)" %
                                   (name, r), re.VERBOSE | reflags)
                    if (c.match("")):
                        print >>sys.stderr, "lex: Regular expression for rule '%s' matches empty string." % name
                        error = 1
                        continue
                except re.error as e:
                    print >>sys.stderr, "lex: Invalid regular expression for rule '%s'. %s" % (
                        name, e)
                    if '#' in r:
    ...
    # No more errors will be generated, but there might be some warning
    # messages.

    # Build the master regular expressions

    for state in regexs.keys():
        lexre, re_text = _form_master_re(
            regexs[state], reflags, ldict, toknames)
        lexobj.lexstatere[state] = lexre
        lexobj.lexstateretext[state] = re_text
        if debug:
            for i in range(len(re_text)):
                print("lex: state '%s'. regex[%d] = '%s'" % (
                    state, i, re_text[i]))
