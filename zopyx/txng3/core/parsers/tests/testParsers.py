...
class EnglishParserTests(unittest.TestCase):
    ...
    def _test(self, query, expected):
        ...
        if expected_lst != got_lst:
            raise AssertionError('\nParser: %s\nquery: %s\ngot:      %s\nexpected: %s\nlexer: %s' % (
                self.p.__class__.__name__, query, got, expected, self.p.lexer(query)))
    ...
    def testFieldsSimple(self):
        ...
        self.assertRaises(QueryParserError, self._test,
                          'somefield : foo', WordNode('somefield', 'foo'))
...
class DumbTestBase(unittest.TestCase):
    ...
    def _test(self, query, expected):
        ...
        if expected_lst != got_lst:
            raise AssertionError('\nParser: %s\nquery: %s\ngot:      %s\nexpected: %s' % (
                self.p.__class__.__name__, query, got, expected))
...
class DumbOrTests(DumbTestBase):
    ...
    def testOr(self):
        self._test('a b c', OrNode(
            (WordNode("a"), WordNode("b"), WordNode('c'))))
