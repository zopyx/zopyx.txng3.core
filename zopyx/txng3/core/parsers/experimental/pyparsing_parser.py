...
TermTruncRight.setParseAction(
    lambda s, loc, toks: TermTruncRightNode(toks[0][:-1]))

TermTruncLeft = Regex(r'\*' + r'\w+', re.UNICODE)
TermTruncLeft.setParseAction(
    lambda s, loc, toks: TermTruncLeftNode(toks[0][1:]))

TermSubstring = Regex('\*' + r'\w+' + '\*', re.UNICODE)
TermSubstring.setParseAction(
    lambda s, loc, toks: TermSubstringNode(toks[0][1:-1]))
...
ListOfTerms = OneOrMore(
    Or([Term, TermSimiliar, TermTruncLeft, TermTruncRight, TermRegex, TermSubstring]))
